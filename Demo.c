...
static TaskHandle_t xTask3_2_3b = NULL;
static TaskHandle_t xExercise2Handle = NULL;
static TaskHandle_t xDrawTaskEx3_2_2 = NULL;
static TaskHandle_t xTaskEx3_2_4 = NULL;
static TaskHandle_t xStaticTask = NULL;
static TaskHandle_t xDynamicTask = NULL;
static TaskHandle_t xToggleTask = NULL;
static TaskHandle_t xTask3_2_3 = NULL;
...
int main() {
	...
	CountQueue = xQueueCreate(1, sizeof(int));
	...
	xTaskCreate(DynamicTask, "DynamicTask", 1000 , NULL , 3 ,&xDynamicTask);
	...
	xTaskCreate(Task3_2_3, "Task3_2_3", 1000 , NULL ,4  ,&xTask3_2_3);
	xTaskCreate(checkButtons, "checkButtons", 1000 , NULL ,4  ,NULL);
	xTaskCreate(Exercise2, "Exercise2", 1000 , NULL ,3  ,&xExercise2Handle);
	xTaskCreate(ResetStateE, "ResetbuttonE_state", 1000 , NULL ,5  ,NULL);
	xTaskCreate(Task3_2_4, "Task3_2_4", 1000 , NULL ,4 , &xTaskEx3_2_4);
	xTaskCreate(ToggleTask3_2_4, "ToggleTask3_2_4", 1000 , NULL ,4 , &xToggleTask);
	xTaskCreate(Task3_2_3_b, "Task3_2_3_b", 1000 , NULL ,4  ,&xTask3_2_3b);
...
void ToggleTask3_2_4(){
	...
	while(1){
		...
		//try and read from queue, if new value then update current_count
		if (xQueueReceive(CountQueue, &input, 0) == pdTRUE)
			current_count = input;


		sprintf(str, "The value of counter is: %d", current_count);
...
void Task3_2_4(){
	...
	int time=1;
	vTaskSuspend(xTaskEx3_2_4);
	xQueueSend(CountQueue, &task3_2_4_counter, 0);
	while(1){
		time++;
		if( time==1000){
		 task3_2_4_counter++;
		...
		//xQueueSend(CountQueue, &task3_2_4_counter, 0);
		//sprintf(str, "The value of counter is: %d", task3_2_4_counter);
		//gdispDrawString(40,100,str,font1,Black);
		//TODO make a queue up top and send counter valuE
		vTaskDelayUntil(&xLastWakeTime, xFrequency);
	}
...
void Exercise2(){
	TickType_t xLastWakeTime;
		...
		font_t font1; // Load font for ugfxxTask3_2_3b
		font1 = gdispOpenFont("DejaVuSans24*");
    	...
    	if( xSemaphore != NULL )
    	    {
    	        ...
    	        sprintf(str, "The total Button A count is: %d ",count );
    	            	        	//gdispClear(White);
    	        gdispDrawString(100, 30 , str, font1, Black);
    	    	vTaskDelayUntil(&xLastWakeTime, xFrequency );
...
void Task3_2_3_b(){
    ...
    while( 1 ){
    	...
    	if(buttonB_state==1){
    		...
    		xTaskNotifyGive(xTask3_2_3b);
    	}

    	if(ulTaskNotifyTake( pdTRUE, 20 ) == pdTRUE ){
    		count++;

    	}
     sprintf(str, "The total Button B count is: %d ",count );
...
void drawTask(){
	...
	vTaskSuspend(xTask3_2_3 );
	vTaskSuspend(xTask3_2_3b );
	vTaskSuspend(xTaskEx3_2_4 );
	vTaskSuspend(xToggleTask );
	...
	while(TRUE) {
		...
		if (counter_E==0)
		{
			...
			vTaskResume(xTask3_2_3);
			vTaskResume(xTask3_2_3b);
			vTaskResume(xTaskEx3_2_4);
			vTaskResume(xToggleTask);
